//! APICs are Advanced Programmable Interrupt Controllers
//! They are used to manage interrupts in a more flexible way than the older PICs (Programmable
//! Interrupt Controllers). This is because we can have multiple APICs in a system, and they can be
//! used to route interrupts to specific
//!
//! For example, we need to remap the PICs to avoid conflicts with the APICs, and we need to
//! initialize the APICs to enable them.
//!
//! Currently the timer interrupt overlaps with the PIT (Programmable Interval Timer) interrupt, so
