; viceOS Boot Stub - 32-bit to 64-bit transition
...
; Simple single-character serial macro
%macro SERIAL_CHAR 1
    mov dx, 0x3F8
    mov al, %1
    out dx, al
...
_start:
	...
	; This means that virtual addresses will match physical addresses, which simplifies the transition
	; TODO: Post transition, we can set up more complex paging if needed (e.g. higher half kernel, like in Windows/Linux)
    ...
    mov edi, pd_table		; point EDI to start of PD
    mov eax, 0x83           ; flags: present + writable + huge (2MB)
.fill_pd:
