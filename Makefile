...
KERNEL_ELF := $(TARGET_DIR)/vice_kernel
BOOT_STUB_OBJ := $(BUILD_DIR)/boot_stub.o
ISO_FILE := $(BUILD_DIR)/viceOS.iso
...
boot_stub:
	...
	@mkdir -p $(BUILD_DIR)
	$(NASM) -f elf64 kernel/src/asm/boot_stub.asm -o $(BOOT_STUB_OBJ)

# Build kernel
kernel: boot_stub
	@echo "Building viceOS kernel..."
	$(CARGO) build $(CARGO_OPTS) --target $(TARGET) -p vice_kernel
	@echo "Linking boot stub with kernel..."
	x86_64-elf-ld -n -T linker/x86_64_direct.ld \
		--gc-sections \
		-o $(KERNEL_ELF) \
		$(BOOT_STUB_OBJ) \
		$(TARGET_DIR)/libvice_kernel.a
...
iso: kernel
	...
	@echo '' >> $(GRUB_DIR)/grub.cfg
	@echo 'menuentry "viceOS" {' >> $(GRUB_DIR)/grub.cfg
...
# Run viceOS with persistent disk
run: iso
	@if [ ! -f $(DISK_IMG) ]; then \
...
# Run clippy
clippy:
	$(CARGO) clippy --target $(TARGET) -p vice_kernel

# Help
help:
	@echo "viceOS Build System"
	@echo ""
